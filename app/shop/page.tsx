    ...
    <main className="min-h-screen bg-[#F7F1E8] px-5 py-20 text-[#29251F] sm:px-8 lg:px-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((product) => <motion.div key={product.slug} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:.4}} className="group overflow-hidden rounded-[2rem] border border-[#29251F]/10 bg-[#FFFDF8] shadow-[0_18px_40px_rgba(58,47,41,0.04)]">
            <Link href={`/shop/${product.slug}`} className="block overflow-hidden"><div className="overflow-hidden"><Image src={product.image} alt={product.name} width={640} height={820} className="h-[420px] w-full object-cover transition duration-700 group-hover:scale-[1.04]" /></div></Link>
            <div className="space-y-4 p-6"><div className="flex items-center justify-between gap-4"><span className="text-[0.62rem] uppercase tracking-[0.26em] text-[#75695B]">{product.category}</span><span className="rounded-full bg-[#EFE5D6] px-2.5 py-1 text-[0.52rem] uppercase tracking-[0.18em] text-[#29251F]">{product.badge}</span></div>
              ...
              <div className="flex items-center justify-between gap-4 border-t border-[#29251F]/10 pt-4"><p className="text-xl font-medium">${product.price}</p><button type="button" onClick={() => addItem({productSlug:product.slug,productName:product.name,price:product.price,image:product.image})} className="rounded-full bg-[#29251F] px-4 py-2 text-[0.68rem] font-medium uppercase tracking-[0.2em] text-[#F7F1E8] transition hover:bg-[#3d3530]">+ Add</button></div>
