...
export default function CheckoutPage() {
  ...
  const whatsappMessage = `Hi LAEL 👋 I would like to place an order.\n\n${items.map((item) => `${item.quantity} × ${item.productName} — $${(item.price * item.quantity).toFixed(2)}`).join("\n")}\n\nTotal: $${total.toFixed(2)}\n\nPlease confirm availability and delivery details.`;
  const whatsappLink = buildWhatsAppLink(whatsappMessage, LAEL_WHATSAPP);
  ...
  return <>
    ...
    <main className="min-h-screen bg-[#F7F1E8] px-5 py-28 text-[#29251F] sm:px-8">
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[1.2fr_.8fr]">
        ...
        <aside className="h-fit rounded-3xl border border-[#29251F]/10 bg-[#FFFDF8] p-6 sm:p-8"><p className="text-xs uppercase tracking-[.2em] text-[#75695B]">Order summary</p><div className="mt-6 space-y-4">{items.map(item => <div key={item.productSlug} className="flex justify-between gap-4 text-sm"><span>{item.productName} × {item.quantity}</span><span>${(item.price * item.quantity).toFixed(2)}</span></div>)}</div><div className="my-6 border-t border-[#29251F]/10" /><div className="flex justify-between font-serif text-2xl"><span>Total</span><span>${total.toFixed(2)}</span></div>{method === "razorpay" ? <button disabled={busy} onClick={pay} className="mt-7 w-full rounded-xl bg-[#29251F] py-4 text-sm font-medium uppercase tracking-[.15em] text-[#F7F1E8] disabled:opacity-50">{busy ? "Preparing payment…" : `Pay $${total.toFixed(2)}`}</button> : <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="mt-7 flex w-full items-center justify-center gap-3 rounded-xl bg-[#25D366] py-4 text-sm font-medium uppercase tracking-[.15em] text-white transition hover:bg-[#20bd5a]"><span className="text-lg">◉</span> Order via WhatsApp</a>}<a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="mt-3 flex w-full items-center justify-center rounded-xl border border-[#25D366]/40 py-3 text-xs font-medium uppercase tracking-[.15em] text-[#188f43] transition hover:bg-[#25D366]/5">Chat about this order · +91 98476 41809</a>{orderNumber && <Link href={`/track-order?order=${encodeURIComponent(orderNumber)}`} className="mt-4 block text-center text-sm underline">Track {orderNumber}</Link>}</aside>
      </div>
    </main>
  </>;
}
